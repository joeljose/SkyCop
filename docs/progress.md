...
- [ ] **Altitude-controller oscillation fix (PR 1b)** — data confirms period-2 oscillation, 19.4 m p95 tick-delta, mean stuck between targets. Fix: (a) flip `compute_target` formula to `s * target + (1-s) * previous_z` so `smoothing=0.15` means strong smoothing (matches yaml reader intuition); (b) add asymmetric hysteresis on `building_near` (raise after N=3 consecutive True, lower after M=10 consecutive False). Unit tests + re-run the three measurement missions, confirm oscillation is gone before merging.
