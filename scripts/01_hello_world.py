"""
Lesson 01 — Hello CARLA World

Connects to the CARLA server, prints world info, spawns a vehicle with
an RGB camera, captures one frame using synchronous mode, and saves it.

Key concepts:
  - Client/World connection
  - Blueprint library
  - Spawning actors (vehicle + sensor)
  - Synchronous mode with world.tick()
  - Proper actor cleanup (important: CARLA leaks GPU memory on sensors)
"""

import carla
import os
import queue

CARLA_HOST = os.environ.get("CARLA_HOST", "carla-server")
CARLA_PORT = int(os.environ.get("CARLA_PORT", 2000))
OUTPUT_DIR = "/app/output"


def main():
    actors = []  # Track all spawned actors for cleanup

    try:
        # ── 1. Connect ─────────────────────────────────────────
        client = carla.Client(CARLA_HOST, CARLA_PORT)
        client.set_timeout(30.0)
        world = client.get_world()

        print(f"Connected to CARLA!")
        print(f"  Map:    {world.get_map().name}")
        print(f"  Maps:   {client.get_available_maps()}")

        # ── 2. Enable synchronous mode ─────────────────────────
        # This ensures world.tick() controls simulation stepping,
        # so sensor data is aligned and nothing is missed.
        original_settings = world.get_settings()
        settings = world.get_settings()
        settings.synchronous_mode = True
        settings.fixed_delta_seconds = 0.05  # 20 FPS
        world.apply_settings(settings)

        # ── 3. Spawn a vehicle ─────────────────────────────────
        bp_lib = world.get_blueprint_library()
        vehicle_bp = bp_lib.find("vehicle.tesla.model3")
        spawn_points = world.get_map().get_spawn_points()

        vehicle = world.try_spawn_actor(vehicle_bp, spawn_points[0])
        if vehicle is None:
            vehicle = world.try_spawn_actor(vehicle_bp, spawn_points[1])
        if vehicle is None:
            raise RuntimeError("Could not spawn vehicle at any point")

        actors.append(vehicle)
        print(f"  Spawned: {vehicle.type_id} (id={vehicle.id})")

        # ── 4. Attach an RGB camera ───────────────────────────
        camera_bp = bp_lib.find("sensor.camera.rgb")
        camera_bp.set_attribute("image_size_x", "1280")
        camera_bp.set_attribute("image_size_y", "720")
        camera_bp.set_attribute("fov", "90")

        # Mount on top of the vehicle, looking forward
        camera_transform = carla.Transform(carla.Location(x=1.5, z=2.4))
        camera = world.spawn_actor(camera_bp, camera_transform, attach_to=vehicle)
        actors.append(camera)

        # Use a queue to receive sensor data synchronously
        image_queue = queue.Queue()
        camera.listen(image_queue.put)

        # ── 5. Tick the world and capture ──────────────────────
        # Tick a few times to let the scene settle (lighting, physics)
        for _ in range(5):
            world.tick()

        # Grab the latest frame
        image = image_queue.get(timeout=5.0)

        os.makedirs(OUTPUT_DIR, exist_ok=True)
        out_path = f"{OUTPUT_DIR}/01_hello_world.png"
        image.save_to_disk(out_path)
        print(f"  Saved: {out_path} ({image.width}x{image.height})")

    finally:
        # ── 6. Cleanup ─────────────────────────────────────────
        # Always destroy actors to free GPU memory.
        # Destroy in reverse order (sensors before vehicles).
        for actor in reversed(actors):
            actor.destroy()

        # Restore original settings (async mode)
        try:
            world.apply_settings(original_settings)
        except Exception:
            pass

    print("Done! Your CARLA setup is working.")


if __name__ == "__main__":
    main()
